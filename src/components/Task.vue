  ...
  <article class="task">
    <div class="task_inner">
      ...
      <div class="task_inner-item task_name">
        <router-link :to="TaskPage" class="lnk">
          {{ taskData.title }}
        </router-link>
      </div>
      ...
      <div class="task_inner-item task_priority" :class="taskData.rank">
        ...
        >
          <path
            d="M7 0L0 3L7 6L14 3L7 0Z"
            fill="#E9B500"
            v-if="taskData.rank == 'medium'"
          />
          <path
            d="M7 8L14 0L7 4L0 0L7 8Z"
            fill="#3CA961"
            v-else-if="taskData.rank == 'low'"
          />
      ...
      <div class="task_btn" v-show="!short">
        <DropMenu>
          ...
          <button
            class="lnk dropdown-content-item"
            @click="status('opened')"
            v-show="taskData.status !== 'opened'"
          >
            Переоткрыть
          </button>
          <button
          ...
          <button
            ...
            v-show="taskData.status !== 'complete'"
          >
            Готово
          </button>
          <button
            ...
            v-show="taskData.status === 'inProgress'"
          >
            На тестирование
          </button>
          <span
...
export default {
  ...
  methods: {
    ...
    deleteTask() {
      console.log("hi");
      api.Events.deleteTask(this.taskData.id).then(() =>
        this.fetchTasks(this.filter)
      );
    },
    status(value) {
      api.Events.changeStatus(this.taskData.id, value).then(() =>
        this.fetchTasks(this.filter)
      );
    },
  },
